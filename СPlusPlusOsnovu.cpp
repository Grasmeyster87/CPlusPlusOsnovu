    ...
    do
    {
        if (values >= 0 && values <= 33)
        {
           ...
           switch (values)
            ...
            // Вывода символов латиницы с их значениями ASCII используя цикла 'while '
            case(5):
            {
...
-            Возвращает итератор на первый вставленный элемент.
            ...
            */
            ...
            - Это **тип элементов**, которые хранятся в контейнере.  
            - Определяется как `typedef T value_type;`, где `T` — параметр шаблона.  
            - Используется для написания обобщённого кода, когда мы не знаем заранее, какой тип хранит контейнер.
            */
            std::vector<int> v_value_type = {1, 2, 3};
            ...
            */
            ...
            cout << "\n---------------------------------------------- size_type --------------------------------------------------------------------------" << endl;
            /*
            В C++ **`size_type`** — это *типовой псевдоним* (type alias), который определяет тип, используемый контейнером для представления количества элементов.  



            ## Основные характеристики
            - **Определение:**  
                 `size_type` — это беззнаковый целочисленный тип (`unsigned`), достаточный для хранения размера контейнера.  
            - **Где встречается:**  
             - В стандартных контейнерах (`std::vector`, `std::list`, `std::map`, и др.).  
             - В аллокаторах (`Allocator::size_type`).  
            - **Назначение:**  
                 Используется для методов контейнеров, возвращающих количество элементов (`size()`, `max_size()`, `capacity()`).
            */

            /*
            std::vector<int> v = {1, 2, 3, 4, 5};

            // Получаем тип size_type
            using size_t_vec = std::vector<int>::size_type;

            size_t_vec n = v.size();  // количество элементов
        ...
        case(138):
        {
            ...
            cout << "\t•	Это двусвязный список — структура данных, где каждый элемент хранит ссылку на предыдущий и следующий.\n\n" <<
                "\t• 	В отличие от  или , элементы списка не лежат подряд в памяти, а связаны указателями.\n\n" <<
                "\t• 	Основное преимущество: быстрая вставка и удаление элементов в любом месте списка (операции выполняются за постоянное время).\n\n" <<
                "\t• 	Недостаток: медленный доступ по индексу — чтобы добраться до элемента, нужно пройти список от начала или конца.\n\n" <<
                "\tОсновные особенности\n\n" <<
                "\t• 	Находится в заголовочном файле .\n\n" <<
                "\t• 	Является шаблонным классом:  — список элементов типа .\n\n" <<
                "\t• 	Поддерживает двунаправленные итераторы (можно идти вперёд и назад).\n\n" <<
                "\t• 	Хорошо подходит для задач, где часто вставляют/удаляют элементы в середине.\n" << endl;
            
            /*           

            🔹 Конструкторы и базовые операции
                - `list()` — создать пустой список  
                - `list(size_type n)` — создать список из `n` элементов  
                - `list(size_type n, const T& value)` — создать список из `n` одинаковых элементов  
                - `list(initializer_list<T>)` — создать список из набора значений  
                - `list(const list&)` — копирующий конструктор  
                - `list(list&&)` — перемещающий конструктор  
            ...
            🔹 Размер и состояние
                - `empty()` — проверка, пустой ли список  
            ...
            🔹 Модификаторы
                ...
                - `insert(pos, value)` — вставить элемент перед `pos`  
                - `emplace(pos, args...)` — создать элемент на месте  
                - `erase(pos)` — удалить элемент по итератору  
                - `push_back(value)` — добавить в конец  
                - `emplace_back(args...)` — создать элемент в конце  
                - `pop_back()` — удалить последний элемент  
                - `push_front(value)` — добавить в начало  
                - `emplace_front(args...)` — создать элемент в начале  
                - `pop_front()` — удалить первый элемент  
                - `resize(n)` — изменить размер списка  
                - `swap(other)` — обменять содержимое с другим списком  


            🔹 Операции со списком
                - `merge(other)` — слить два отсортированных списка  
                - `splice(pos, other)` — переместить элементы из другого списка  
                - `remove(value)` — удалить все элементы с данным значением  
                - `remove_if(pred)` — удалить элементы по условию  
                - `reverse()` — развернуть список  
                - `sort()` — отсортировать список  
                - `unique()` — удалить подряд идущие одинаковые элементы  

            🔹 Аллокатор
                - `get_allocator()` — получить объект аллокатора
            */
            ...
            /*
            Функция `push_back` добавляет новый элемент **в конец списка**. Для двусвязного списка нужно:
                1. Создать новый узел.
                2. Если список пуст — новый узел становится и `head`, и `tail`.
                3. Если список не пуст — связать новый узел с текущим `tail` и обновить `tail`.
            */
            ...
            /*
            Таким образом, `push_front` добавляет элементы в начало списка, корректно обновляя связи `prev` и `next`.
            */
            ...
            `pop_front` удаляет элемент **с начала списка**:
                1. Если список пуст — ничего не делаем (или можно выбросить исключение).
                2. Если в списке один элемент — удаляем его и обнуляем `head` и `tail`.
                3. Если элементов больше одного — переносим `head` на следующий узел, удаляем старый `head`, и обновляем связи.
            */
            ...
            🔹 Логика работы
                `pop_back` удаляет элемент **с конца списка**:
                    ...
                    2. Если в списке один элемент — удаляем его и обнуляем `head` и `tail`.  
                    3. Если элементов больше одного — переносим `tail` на предыдущий узел, удаляем старый хвост, и обновляем связи.
            */
            ...
            cout << "\t Вывод количество елементов в нашем контейнере - " << mylist138.size() << endl << endl;
            ...
            - Работает с любыми контейнерами, поддерживающими итераторы (например, `vector`, `list`, `deque`).
            - **Не удаляет элементы физически**, а лишь "сдвигает" уникальные элементы в начало диапазона и возвращает итератор на новую границу.
            - Чтобы реально удалить лишние элементы, нужно вызвать метод `erase()` контейнера.
            */
            mylist138.unique();
            ...
            /*
            - Доступен у всех стандартных контейнеров STL (`vector`, `list`, `map`, `set` и др.).
            - Возвращает **итератор**, который указывает на первый элемент.
            - Если контейнер пустой, `begin()` будет равен `end()`.
            - Есть версии:
              - `begin()` — возвращает обычный итератор.
              - `cbegin()` — возвращает константный итератор (нельзя изменять элементы).
              - Для ассоциативных контейнеров (`map`, `set`) итератор указывает на первый элемент в порядке сортировки.
            */
            ...
            /*
            - Доступен у контейнеров: `vector`, `list`, `deque`, `map`, `set` и др.
            - Возвращает итератор на элемент, следующий за удалённым.
            - Может удалять:
                 - один элемент по итератору,
                 - диапазон элементов `[first, last)`
            */
            ...
            /*
            remove - удаляет все элементы списка, равные заданному значению (сравнение через `operator==`). 
            Операция изменяет контейнер непосредственно и не возвращает значение.
            */
            mylist138.remove(15);
            ...
            /*
            assign заменяет содержимое списка новыми элементами. 
            Существующие элементы удаляются, а контейнер получает новые элементы, заданные одним из доступных способов.
            */
            ...
            PrintList138(mylist138);
            cout << endl << endl;
            cout << " ======================================== merge(other) ==========================================================" << endl << endl;
			cout << "\tВывод на списка после функции mylist.merge(other)\n\n\t - слияние двух отсортированных списков\t";
            /*
            - Условия:
                 - Оба списка (`this` и `other`) должны быть **отсортированы** по одному и тому же критерию (по умолчанию — `operator<`).
                 - После вызова `merge`, элементы из `other` перемещаются в текущий список.  
                 - Список `other` становится пустым.
                 - **Сложность**: линейная, \(O(n+m)\), где \(n\) и \(m\) — размеры списков.
                 - **Особенность**: операция выполняется через перемещение узлов, без копирования элементов.
            */
            std::list<int> a_merge = { 1, 3, 5, 7 };
            std::list<int> b_merge = { 2, 4, 6, 8 };

            struct Desc {
                bool operator()(int x, int y) const {
                    return x > y; // сортировка по убыванию
                }
            };
			//a_merge.merge(b_merge Desc()); // объединение по убыванию Вариант с компаратором

            a_merge.merge(b_merge);  // объединяет a и b
            ...
            std::cout << "Размер b: " << b_merge.size() << "\n"; // 0

            cout << endl << endl;
            cout << " ======================================== splice(pos, other) ==========================================================" << endl << endl;
            /*
            Важные особенности
            • 	Нет копирования или перемещения: элементы просто меняют список, оставаясь теми же объектами в памяти.
            • 	Сложность операции: постоянная для одного элемента, линейная по числу переносимых элементов.
            • 	Безопасность: итераторы на перемещаемые элементы остаются валидными, но итераторы на старые позиции в исходном списке становятся недействительными.

            Когда использовать
            • 	Когда нужно объединить списки без лишних копирований.
            • 	Для перемещения отдельных элементов между списками.
            • 	Для реализации алгоритмов, где важна скорость работы с узлами (например, при написании собственных структур данных поверх ).
            */

            list<int> l1_splice = { 1, 2, 5 };
            list<int> l2_splice = { 4, 3 };

            // Переносим все элементы l2 в начало l1
            l1_splice.splice(l1_splice.begin(), l2_splice);

            for (int x : l1_splice) cout << x << " ";

            cout << endl << endl;
            cout << " ======================================== remove_if(pred) ==========================================================" << endl << endl;
            /*
            remove_if — это метод контейнера std::list (и некоторых других контейнеров, например std::forward_list), 
            который удаляет элементы, удовлетворяющие заданному предикату.
            list.remove_if(UnaryPredicate pred);
            Особенности
            • 	Итераторы: итераторы на удалённые элементы становятся недействительными, остальные остаются валидными.
            • 	Возвращаемое значение: ничего не возвращает (в отличие от алгоритма , который работает с диапазонами и возвращает итератор).
            */

            list<int> l_remove_if = { 1, 2, 3, 4, 5, 6 };

            // Удаляем все чётные числа
            l_remove_if.remove_if([](int x) { return x % 2 == 0; });

            for (int x : l_remove_if) cout << x << " ";
            ...
            /*
            У мові програмування C++, функція front() для контейнера std::list — це швидкий і зручний спосіб отримати доступ до першого елемента списку.*/

            std::list<int> numbers_front = { 10, 20, 30, 40 };

            // 1. Отримання значення
            std::cout << "Перший елемент: " << numbers_front.front() << std::endl;

            // 2. Зміна значення через посилання
            numbers_front.front() = 100;
            ...
            */
            ...
            /*
            Метод empty() — це фундаментальна функція в C++, яка використовується для перевірки стану контейнера. 
            Вона дає просту відповідь: чи є в списку хоча б один елемент.
            Що робить empty()?

            Ця функція повертає логічне значення (bool):

             true — якщо список порожній (розмір дорівнює 0).

             false — якщо в списку є хоча б один вузол.

            */
            std::list<int> data_empty;

            // Перевірка перед роботою
            if (data_empty.empty()) {
                std::cout << "Список порожній! Додаємо елемент..." << std::endl;
                data_empty.push_back(42);
            }

            // Тепер безпечно викликати front()
            if (!data_empty.empty()) {
                std::cout << "Перший елемент: " << data_empty.front() << std::endl;
            }
            ...
            /*
            Функція max_size() повертає максимально можливу кількість елементів, яку теоретично може вмістити контейнер std::list у вашій системі.

            Чому max_size() у списку менший, ніж у vector?

            Якщо ви порівняєте max_size() для std::vector<int> та std::list<int>, ви помітите, що у списку це число менше.

            Це пояснюється структурою двозв'язного списку:

            Кожен елемент списку — це не просто дані (int).

            Це вузол, який містить саме значення + два вказівники (на наступний і попередній елементи).

            Через ці додаткові вказівники кожен вузол займає більше пам'яті, ніж один елемент у векторі, тому загальна кількість вузлів,
            які можуть "вміститися" в адресний простір, є меншою.

            */

            std::list<int> my_list_max_size;

            std::cout << "Максимально можливий розмір списку: "
                << my_list_max_size.max_size() << " елементів." << std::endl; // 768614336404564650 елементів.
            ...
            /*
            У C++ стандартна бібліотека `std::list` має метод `insert`, який дозволяє вставляти елементи в довільне місце списку.
            ...
            ### Пояснення
            - **`pos`** – ітератор, що вказує на позицію, перед якою буде вставлено новий елемент.
            - **`value`** – значення, яке потрібно вставити.
            - **Повертає** – ітератор на щойно вставлений елемент.
             Додаткові варіанти
            - `insert(pos, count, value)` – вставляє кілька копій значення.
            - `insert(pos, first, last)` – вставляє діапазон елементів з іншого контейнера.
            - `insert(pos, initializer_list)` – вставляє список значень.
            */

            std::list<int> numbers_insert = { 1, 2, 4, 5 };

            // Знайдемо позицію перед елементом "4"
            auto it_insert = std::find(numbers_insert.begin(), numbers_insert.end(), 4);

            // Вставимо число 3 перед "4"
            numbers_insert.insert(it_insert, 3);

            // Виведемо список
            for (int n : numbers_insert) {
                std::cout << n << " ";
            }
            ...
            */
            ...
            /*
            Функція `resize(n)` у контейнері **`std::list`** в C++ використовується для зміни кількості елементів у списку.  

            ### Як працює `resize(n)`:
            - Якщо **поточний розмір > n** → видаляються зайві елементи, залишаються тільки перші `n`.
            - Якщо **поточний розмір < n** → додаються нові елементи в кінець списку.
              - За замовчуванням вони створюються через **конструктор за замовчуванням**.
              - Якщо передати другий параметр `val`, нові елементи будуть ініціалізовані значенням `val`.
            */

            list<int> myList_resize = { 1, 2, 3 };

            myList_resize.resize(5);       // тепер список: 1, 2, 3, 0, 0
            myList_resize.resize(7, 9);    // тепер список: 1, 2, 3, 0, 0, 9, 9
            myList_resize.resize(3);       // тепер список: 1, 2, 3

            for (int x : myList_resize) cout << x << " ";
            ...
            /*
            Функція **`swap(other)`** у контейнері **`std::list`** в C++ використовується для обміну вмістом двох списків.  

            ### Основні моменти:
            - Вона міняє місцями елементи між двома списками.
            - Операція виконується за **O(1)** (постійний час), оскільки відбувається лише обмін внутрішніх структур, а не копіювання елементів.
            - Після виклику `swap`, кожен список містить елементи іншого.
            */

            list<int> listA_swap = { 1, 2, 3 };
            list<int> listB_swap = { 10, 20, 30, 40 };

            cout << "Перед swap:\n";
            for (int x : listA_swap) cout << x << " ";  // 1 2 3
            cout << "\n";
            for (int x : listB_swap) cout << x << " ";  // 10 20 30 40
            cout << "\n";

            listA_swap.swap(listB_swap);

            cout << "Після swap:\n";
            for (int x : listA_swap) cout << x << " ";  // 10 20 30 40
            cout << "\n";
            for (int x : listB_swap) cout << x << " ";  // 1 2 3
            cout << "\n";

            cout << endl << endl;

            cout << " ========================================= rbegin() =========================================================" << endl << endl;
            /*
            Функция **`std::list::rbegin()`** в C++ возвращает **обратный итератор**, 
            который указывает на последний элемент списка. Она используется для обхода контейнера `std::list` в обратном порядке — от конца к началу.  
            */
            list<int> lis_rbegin = { 10, 20, 30, 40, 50 };

            cout << "Список в обратном порядке: ";
            for (auto it = lis_rbegin.rbegin(); it != lis_rbegin.rend(); ++it) {
                cout << *it << " ";
            }

            cout << endl << endl;

            cout << " ========================================= cbegin() =========================================================" << endl << endl;
            /*
            Функция **`std::list::cbegin()`** в C++ возвращает **константный итератор** на первый элемент списка. 
            В отличие от обычного `begin()`, этот итератор не позволяет изменять элементы, на которые он указывает.
            */
            list<int> lis_cbegin = { 10, 20, 30, 40, 50 };

            cout << "Элементы списка: ";
            for (auto it = lis_cbegin.cbegin(); it != lis_cbegin.cend(); ++it) {
                cout << *it << " ";
                // *it = 100;  // Ошибка! Нельзя изменять значение через const_iterator
            }

            cout << endl << endl;

            cout << " ========================================= crbegin() =========================================================" << endl << endl;
            /*
            Функция **`std::list::crbegin()`** в C++ возвращает **константный обратный итератор**, который указывает на последний элемент списка (то есть на начало списка в обратном порядке). В отличие от `rbegin()`, через `crbegin()` нельзя изменять элементы, на которые он указывает.  
            ...
            - **Возвращаемое значение:** константный обратный итератор, указывающий на последний элемент списка.
            - **Если список пуст:** возвращаемый итератор равен `crend()`.
            */

            list<int> lis_crbegin = { 10, 20, 30, 40, 50 };

            cout << "Список в обратном порядке (через crbegin): ";
            for (auto it = lis_crbegin.crbegin(); it != lis_crbegin.crend(); ++it) {
                cout << *it << " ";
                // *it = 100;  // Ошибка! Нельзя изменять значение через const_reverse_iterator
            }
            cout << endl << endl;

            cout << " ========================================= crend() =========================================================" << endl << endl;
            /*
            Функция **`std::list::crend()`** в C++ возвращает **константный обратный итератор**, который указывает на элемент **перед первым элементом списка** (то есть конец диапазона при обратном обходе). Она используется вместе с `crbegin()` для безопасного прохода списка в обратном порядке, без возможности изменения элементов.  
            ...
            - **Синтаксис:**
              ...
              ```
            - **Параметры:** отсутствуют.
            - **Возвращаемое значение:** константный обратный итератор, указывающий на позицию "за пределами" начала списка.
            - **Если список пуст:** возвращаемый итератор равен `crbegin()`.
            */
            list<int> lis_crend = { 10, 20, 30, 40, 50 };

            cout << "Обратный обход (через crbegin/crend): ";
            for (auto it = lis_crend.crbegin(); it != lis_crend.crend(); ++it) {
                cout << *it << " ";
                // *it = 100;  // Ошибка! Нельзя изменять значение через const_reverse_iterator
            }
            ...
            /*
            В C++ класс `std::list` имеет перегруженный оператор присваивания `operator=`, который позволяет заменить содержимое одного списка содержимым другого. Он работает в нескольких вариантах:

            ### Основные формы `operator=`
            | Форма | Описание |
            |-------|-----------|
            | `list& operator=(const list& other);` | Копирующее присваивание: копирует все элементы из другого списка. |
            | `list& operator=(list&& other) noexcept;` | Перемещающее присваивание: переносит элементы из другого списка без копирования (C++11 и новее). |
            | `list& operator=(std::initializer_list<T> ilist);` | Присваивание из списка инициализации: заменяет содержимое значениями из `{...}` (C++11 и новее). |

            ### Особенности
            - После вызова `operator=` текущий список полностью очищается и заменяется новыми элементами.
            - Аллокатор (allocator), используемый контейнером, сохраняется и не меняется при копирующем присваивании.
            - При перемещающем присваивании элементы просто "передаются" другому объекту, что быстрее, чем копирование. После этого исходный список становится пустым.
            - При использовании `initializer_list` можно удобно задавать значения прямо в коде.
            */
            list<int> a_operator = { 1, 2, 3 };
            list<int> b_operator;

            b_operator = a_operator; // копирующее присваивание
            b_operator = { 10, 20, 30 }; // присваивание из initializer_list
            list<int> c_operator;
            c_operator = move(b_operator); // перемещающее присваивание

            for (int x : c_operator) cout << x << " "; // Вывод: 10 20 30

            cout << endl << endl;

            cout << " ========================================= Конструкторы и базовые операции =========================================================" << endl << endl;
            /*
            🔹 Конструкторы и базовые операции
            - `list()` — создать пустой список  
            - `list(size_type n)` — создать список из `n` элементов  
        ...
        case(139):
        {
            ...
            list<int> mylist139 = { 99,15,64,99,979,54,99,99,99,100 };
        ...
        case(140):
        {
            ...
            /*
            std::forward_list — это контейнер из стандартной библиотеки C++, представляющий **односвязный список**. 
            Он был добавлен в C++11 и отличается от `std::list` тем, что хранит ссылки только на следующий элемент, а не на предыдущий.  


            ### 🔑 Основные характеристики
            - **Структура данных**: односвязный список.
            - **Память**: каждый элемент хранит значение и указатель на следующий элемент.
            - **Итераторы**: только **однонаправленные** (forward iterators).
            - **Эффективность**: операции вставки и удаления выполняются быстро, но только при известной позиции (через итератор).
            - **Минус**: нет доступа к элементам по индексу (как в `vector` или `deque`).
            */
        ...
        case(141):
        {
            ...
            /*
            - В отличие от `std::vector`, размер массива задаётся на этапе компиляции и не может изменяться.
            - Поддерживает методы: `.size()`, `.front()`, `.back()`, `.fill()`, `.at()`, а также итераторы.
            - Можно использовать в алгоритмах стандартной библиотеки (`std::sort`, `std::for_each` и т.д.).
        ...
        case(142):
        {
            ...
            std::array<int, 4> arr_142 = { 1, 3, 5, 7 };
            std::array<int, 4> arr1_142 = { 1, 3, 5, 7 };
            std::array<int, 4> arr2_142 = { 2, 4, 6, 8 };

            bool result1 = (arr_142 == arr1_142); // true, так как все элементы совпадают
            bool result2 = (arr_142 > arr1_142);  // false, они равны
            bool result3 = (arr_142 < arr2_142);  // true, так как 1 < 2 на первом элементе

			std::cout << std::boolalpha;  //  это манипулятор вывода в C++ , который позволяет выводить логические значения true и false вместо 1 и 0.
            std::cout << "arr_142 == arr1_142: " << result1 << "\n";
            std::cout << "arr_142 > arr1_142: " << result2 << "\n";
            std::cout << "arr_142 < arr2_142: " << result3 << "\n";

            cout << "\tВыводим результат сравнения двух массивов - " << result1 << endl << endl;
            cout << "\tВыводим результат сравнения двух массивов - " << result2 << endl << endl;
            cout << "\tВыводим результат сравнения двух массивов - " << result3 << endl << endl;
        ...
        case(143):
        {
            ...
            /*
            **`std::deque`** (double-ended queue) — это последовательный контейнер, который позволяет эффективно добавлять и удалять элементы как с начала, 
                            так и с конца. Он похож на `std::vector`, но имеет дополнительные возможности работы с обоими концами.
            ...
            `insert(pos, val)`                    Вставляет элемент в указанную позицию 
            `erase(pos)`                          Удаляет элемент по позиции 
            `clear()`                             Удаляет все элементы 
            `size()`                              Возвращает количество элементов 
            `empty()`                             Проверяет, пуст ли контейнер 
            `operator[]`                          Доступ к элементу по индексу (без проверки) 
            `at(index)`                           Доступ к элементу по индексу (с проверкой) 
            `front()`                             Возвращает первый элемент 
            `back()`                              Возвращает последний элемент 
            `begin()` / `end()`                   Итераторы на начало и конец 
            `rbegin()` / `rend()`                 Обратные итераторы 
            `resize(n)`                           Изменяет размер контейнера 
            `assign(n, val)`                      Заполняет контейнер `n` элементами со значением `val` 
            `emplace_front()` / `emplace_back()`  Конструируют элемент на месте в начале/конце 
            `swap()`                              Обменивает содержимое двух `deque` 
            */
            ...
            */
            ...
            cout << endl << endl;
            cout << " =========================================== erase(pos) =======================================================" << endl << endl;
            /*
            Метод `erase` у контейнера `std::deque` принимает **итератор** (или диапазон итераторов), а не индекс. То есть нельзя напрямую передать число `0`. 
            // Удаление первого элемента
            dq.erase(dq.begin());

            // Удаление элемента по индексу 2 (третий элемент)
            dq.erase(dq.begin() + 2);
            ...
            */
            cout << "Первый элемент: " << dq143.front() << "\n";
            dq143.front() = 991; // можно изменить первый элемент
            std::cout << "После изменения: " << dq143.front() << "\n";
            ...
            */
            std::cout << "Последний элемент: " << dq143.back() << "\n";
            ...
            */
            ...
            cout << " =========================================== end() =======================================================" << endl << endl;
            /*
            Функция **`end()`** в контейнере `std::deque` возвращает итератор, указывающий **на элемент, следующий за последним**.
            Этот элемент не существует, поэтому итератор используется только как "граница" для перебора контейнера.
            */

            for (auto it_end = d143.begin(); it_end != d143.end(); ++it_end) {
                cout << *it_end << " ";
            }
            ...
            cout << " =========================================== rbegin() =======================================================" << endl << endl;
            /*
            Функция `rbegin()` в контейнере **`std::deque`** используется для получения **обратного итератора**, который указывает на последний элемент дека. 
            Это позволяет проходить по контейнеру в обратном порядке — от конца к началу.
            */

            cout << "Deque в обратном порядке: ";
            for (auto it_rbegin = dq143.rbegin(); it_rbegin != dq143.rend(); ++it_rbegin) {
                cout << *it_rbegin << " ";
            }
            ...
            cout << " =========================================== rend() =======================================================" << endl << endl;
            /*
            Функция **`rend()`** в контейнере `std::deque` возвращает **обратный итератор**, указывающий на элемент **перед первым элементом** дека. 
            Она используется как граница конца при обратной итерации. 
            */

            cout << "Обратный обход: ";
            for (auto it_rend = dq143.rbegin(); it_rend != dq143.rend(); ++it_rend) {
                cout << *it_rend << " ";
            }
            ...
            */
            ...
            cout << " =========================================== operator[] =======================================================" << endl << endl;
            /*
            Оператор **`operator[]`** в контейнере `std::deque` предоставляет доступ к элементам по индексу, аналогично массиву. 
            */

            // доступ к элементам
            cout << "Первый элемент: " << dq143[0] << endl;
            cout << "Третий элемент: " << dq143[2] << endl;

            // изменение значения
            dq143[1] = 99;
            cout << "После изменения: ";
            for (int x : dq143) cout << x << " ";
            cout << endl;

            // безопасный доступ
            try {
                cout << dq143.at(10) << endl; // вызовет исключение
            }
            ...
            cout << " =========================================== at() =======================================================" << endl << endl;
            /*
            Функция `at()` в контейнере **`std::deque`** используется для доступа к элементам по индексу с проверкой границ. 
            В отличие от оператора `[]`, она гарантирует безопасность: если индекс выходит за пределы контейнера, 
            будет выброшено исключение `std::out_of_range`
            */
            try {
                std::cout << "Элемент с индексом 2: " << d143.at(2) << std::endl; // 30
                std::cout << "Элемент с индексом 5: " << d143.at(5) << std::endl; // Ошибка
            }
        ...
        case(144):
        {
            ...
            /*
            Контейнер **`std::set`** в C++ — это ассоциативный контейнер, который хранит **уникальные элементы** в отсортированном порядке. 
            Внутри он реализован как **сбалансированное бинарное дерево поиска (Red-Black Tree)**, что обеспечивает операции поиска, 
            вставки и удаления за время **O(log n)**.  

                ### Основные команды (методы) `std::set`

                #### 1. Конструкторы и присваивание
                - `set s;` — создать пустое множество.
                - `set s = {1, 2, 3};` — инициализация списком.
                - `s = other;` — оператор присваивания.

                #### 2. Модификация
                - `insert(value)` — вставка элемента.
                - `emplace(args...)` — конструирование и вставка элемента на месте.
                ...
                #### 3. Доступ и поиск
                - `find(value)` — возвращает итератор на элемент или `end()`, если не найден.
                - `count(value)` — возвращает `1`, если элемент есть, иначе `0`.
                - `lower_bound(value)` — итератор на первый элемент, не меньший `value`.
                - `upper_bound(value)` — итератор на первый элемент, больший `value`.
                - `equal_range(value)` — пара итераторов `[lower_bound, upper_bound)`.

                #### 4. Размер и состояние
                - `size()` — количество элементов.
                - `empty()` — проверка на пустоту.
                - `max_size()` — максимальное количество элементов (теоретический предел).
            ...
            */
            /*
            Контейнер **`std::multiset`** в C++ похож на `std::set`, но с ключевым отличием: он допускает хранение **дубликатов элементов**. 
            Все элементы остаются отсортированными, но уникальность не гарантируется.  

                ### Основные особенности
                - **Структура:** реализован как сбалансированное бинарное дерево поиска (обычно красно-черное дерево).  
                - **Порядок:** элементы автоматически сортируются по заданному компаратору (по умолчанию — `std::less<T>`).  
                - **Дубликаты:** допускает несколько одинаковых значений.  
                - **Сложность операций:** вставка, удаление и поиск выполняются за **O(log n)**.  

                ---

                ### Основные методы `std::multiset`

                #### 1. Конструкторы и присваивание
                - `multiset m;` — создать пустое множество.  
                - `multiset m = {1, 2, 2, 3};` — инициализация списком.  

                #### 2. Модификация
                - `insert(value)` — вставка элемента (может быть дубликат).  
                - `emplace(args...)` — конструирование и вставка элемента на месте.  
                - `erase(value)` — удаление всех элементов с данным значением.  
                - `erase(iterator)` — удаление по итератору.  
                - `clear()` — очистка контейнера.  
                - `swap(other)` — обмен содержимым.  

                #### 3. Поиск и доступ
                - `find(value)` — итератор на первый элемент с данным значением.  
                - `count(value)` — количество элементов с данным значением.  
                - `lower_bound(value)` — итератор на первый элемент, не меньший `value`.  
                - `upper_bound(value)` — итератор на первый элемент, больший `value`.  
                - `equal_range(value)` — пара итераторов, охватывающая все элементы равные `value`.  

                #### 4. Размер и состояние
                - `size()` — количество элементов.  
                - `empty()` — проверка на пустоту.  
                - `max_size()` — теоретический предел размера.  
            ...
            */
            ...
            cout << " ============================================ Set - emplace(args...) ======================================================" << endl << endl;
            /*
            В C++ контейнер **`std::set`** имеет метод **`insert()`**, который используется для добавления элемента в множество.  

            ### Основные моменты:
            - `std::set` хранит элементы **в отсортированном порядке** и **без дубликатов**.  
            - При вызове `insert()`:
              - Если элемент **уже существует**, он **не будет добавлен**.  
              - Если элемент **новый**, он будет вставлен в правильное место (с учётом сортировки).  
            */
            struct Person {
                std::string name;
                int age;
            ...
            // insert — создаём объект заранее
            people.insert(Person{ "Alice", 30 });

            // emplace — объект создаётся прямо внутри set
            ...
            cout << " ============================================ Set - erase(value) ======================================================" << endl << endl;
            /*
            ...
            */
            ...
            cout << " ============================================ Set - erase(iterator) ======================================================" << endl << endl;
            /*
            ...
            */
            ...
            cout << " ============================================ Set - rbegin() ======================================================" << endl << endl;
            /*
             std::set имеет функцию rbegin(), которая возвращает **обратный итератор**, указывающий на последний элемент множества.
            */
           ...
           for (auto it = mySet.rbegin(); it != mySet.rend(); ++it) {
            ...
            // Доступ к последнему элементу множества
            cout << "Последний элемент: " << *mySet.rbegin() << endl;

            cout << endl << endl;

            cout << " ============================================ Set - rend() ======================================================" << endl << endl;
            /*
            rend() в контейнере **`std::set`** (и других стандартных контейнерах C++) возвращает **обратный итератор**, который указывает **за пределы первого элемента** множества при обходе в обратном порядке.
            */
            ...
            cout << " ============================================ Set - max_size() ======================================================" << endl << endl;
            /*
            set имеет функцию-член **`max_size()`**, которая возвращает максимально возможное количество элементов, которое данный контейнер может хранить.
            */
            set<int> s_max_size;
            cout << "Максимальный размер: " << s_max_size.max_size() << endl;

            cout << endl << endl;

            cout << " ============================================ Set - empty() ======================================================" << endl << endl;
            /*
            empty() в контейнере `std::set` используется для проверки, пуст ли контейнер.
            */
            ...
            if (!mySet.empty()) {
                cout << "Множество содержит элементы" << endl;
            }
            cout << endl << endl;

            cout << " ============================================ Set - size() ======================================================" << endl << endl;
            /*
            size() в контейнере `std::set` возвращает текущее количество элементов в множестве. 
            */
            cout << "Размер множества: " << mySet.size() << endl;

            cout << endl << endl;

            cout << " ============================================ Set - count(value) ======================================================" << endl << endl;
            /*
            Функция-член count(value) в контейнере `std::set` используется для проверки, содержится ли указанный элемент в множестве.
            В `std::set` всегда будет либо `0`, либо `1`, потому что множество хранит только уникальные элементы.
            */
            cout << "Есть ли 2? " << mySet.count(2) << endl;
            cout << "Есть ли 5? " << mySet.count(5) << endl;

            cout << endl << endl;

            cout << " ============================================ Set - lower_bound(value) ======================================================" << endl << endl;
            /*
            Возвращает итератор на первый элемент множества, который **не меньше** (т.е. равен или больше) указанного значения.
            */
            ...
            cout << " ============================================ Set - upper_bound(value) ======================================================" << endl << endl;
            /*
            ...
            */
            ...
            cout << " ============================================ Set - equal_range(value) ======================================================" << endl << endl;
            /*
            ...
            */
            ...
            cout << " ============================================ Multiset - multiset m; ======================================================" << endl << endl;
            /*
                - `multiset m;` — создать пустое множество. 
            */
            std::multiset<int> ms_144;

            if (ms_144.empty()) {
                cout << "Мультисет пустой" << endl;
            }
            ...
            cout << " ============================================ Multiset -  m = {1, 2, 2, 3}; ======================================================" << endl << endl;
            /*
            `multiset m = {1, 2, 2, 3};` — инициализация списком.
            */
            std::multiset<int> ms_144_1 = { 1, 2, 2, 3 };
            ...
            cout << " ============================================ Multiset - insert(value) ======================================================" << endl << endl;
            /*
            `insert(value)` — вставка элемента (может быть дубликат).
            */
            // Добавляем новые элементы
            ms_144_1.insert(2);   // ещё один дубликат
            ms_144_1.insert(4);   // новый элемент
            ...
            cout << " ============================================ Multiset - emplace(args...) ======================================================" << endl << endl;
            /*
            `emplace(args...)` в `std::multiset` — это метод, 
            который принимает аргументы конструктора элемента и создаёт объект **непосредственно внутри контейнера**, 
            без промежуточного временного объекта. 
            */

            ms_144_1.emplace(10); // создаётся элемент со значением 10
            ms_144_1.emplace(5);  // создаётся элемент со значением 5
            ms_144_1.emplace(10); // дубликаты разрешены
            ...
            cout << " ============================================ Multiset - emplace(args...) ======================================================" << endl << endl;
            /*
            В `std::multiset` метод `erase(value)` удаляет **все элементы**, равные указанному значению.
            */
            ...
            cout << " ============================================ Multiset - erase(iterator) ======================================================" << endl << endl;
            /*
            `erase(iterator)` в `std::multiset` удаляет **один элемент**, на который указывает данный итератор. 
            Это отличается от `erase(value)`, где удаляются все элементы с указанным значением.
            */

            // Находим первый элемент со значением 2
            auto it_multiset_erase = ms_144_1.find(2);

            if (it_multiset_erase != ms_144_1.end()) {
                ms_144_1.erase(it_multiset_erase); // удаляем только один экземпляр "2"
            }
            ...
            cout << " ============================================ Multiset - clear() ======================================================" << endl << endl;
            /*
            Метод `clear()` у контейнера `std::multiset` полностью очищает его содержимое — удаляет **все элементы**. После вызова контейнер становится пустым.
            */
            std::cout << "Размер до clear: " << ms_144_1.size() << "\n";

            ms_144_1.clear(); // удаляем все элементы

            std::cout << "Размер после clear: " << ms_144_1.size() << "\n";
            ...
            */
            ...
            cout << " ============================================ Multiset - count(value) ======================================================" << endl << endl;

            /*
            Что делает `count(value)`
            - Возвращает количество элементов, равных `value`.
            - В отличие от `find`, который возвращает итератор на первый найденный элемент, `count` сразу даёт число совпадений.
            - Время работы: **O(log n + k)**, где `k` — количество найденных элементов.
            */
            std::multiset<int> ms_count = { 1, 2, 2, 3, 4, 2 };

            std::cout << "Количество элементов '2': " << ms_count.count(2) << "\n";
            std::cout << "Количество элементов '5': " << ms_count.count(5) << "\n";
            ...
            */
            ...
            cout << " ============================================ Multiset - max_size() ======================================================" << endl << endl;

            /*
            В `std::multiset` метод **`max_size()`** возвращает максимально возможное количество элементов, которое контейнер теоретически может содержать.  

            ### Особенности
            - Возвращаемый тип: `size_type` (обычно `std::size_t`).  
            - Значение зависит от реализации и доступной памяти, но обычно очень велико (например, порядка \(10^{18}\) на 64-битных системах).  
            - Это **не ограничение, которое вы реально достигнете**, а верхняя граница, определяемая аллокатором и архитектурой.  
            - Время выполнения: **O(1)**.
            */

            std::multiset<int> ms_max_size;

            std::cout << "Максимальный размер multiset: " << ms_max_size.max_size() << "\n";
            ...
            */
            ...
            cout << " ============================================ Multiset - rend() ======================================================" << endl << endl;
            /*
            В `std::multiset` функция **`rend()`** возвращает *обратный итератор*, который указывает на позицию **перед первым элементом** 
            контейнера при обратном обходе.
            */
            std::multiset<int> ms_rend = { 5, 1, 3, 3, 2 };

            // Обход в обратном порядке
            for (auto it = ms_rend.rbegin(); it != ms_rend.rend(); ++it) {
                std::cout << *it << " ";
            }
        ...
        case(145):
        {
            ...
            | `emplace(key, value)`  | Конструирование и вставка элемента                                   |
            | `erase(key)`           | Удаление по ключу                                                    |
            | `erase(iterator)`      | Удаление по итератору                                                |
            | `find(key)`            | Поиск элемента по ключу (возвращает итератор)                        |
            | `count(key)`           | Количество элементов с данным ключом (для `multimap` может быть > 1) |
            | `lower_bound(key)`     | Первый элемент с ключом ≥ указанного                                 |
            | `upper_bound(key)`     | Первый элемент с ключом > указанного                                 |
            | `equal_range(key)`     | Диапазон элементов с данным ключом                                   |
            | `begin()` / `end()`    | Итераторы на начало и конец                                          |
            | `rbegin()` / `rend()`  | Обратные итераторы                                                   |
            | `size()`               | Количество элементов                                                 |
            | `empty()`              | Проверка на пустоту                                                  |
            ...
            */
            ...
            cout << " ===================================================  Map, Multimap - insert({key, value})===============================================" << endl << endl;

            /*
            • 	В  ключи уникальны:
            • 	Если вставить элемент с уже существующим ключом, вставка будет проигнорирована.
            ...
            */
            ...
            cout << " ===================================================  Map, Multimap - erase(key) ===============================================" << endl << endl;
            /*
            оманда **`erase(key)`** в контейнерах `map` и `multimap` используется для удаления элементов по заданному ключу.  

            ## 📌 Поведение в `map`
            - Так как ключи уникальны, `erase(key)` удаляет **не более одного элемента**.
            - Возвращает количество удалённых элементов (`0` или `1`).
            */
            std::multimap<int, std::string> mm;
            mm.insert({ 1, "один" });
            mm.insert({ 1, "ещё один" });
            mm.insert({ 2, "два" });
            ...
            */
            ...
            cout << " ===================================================  Map, Multimap - find(key) ===============================================" << endl << endl;
            /*
            Метод **`find(key)`** в контейнерах `std::map` и `std::multimap` используется для поиска элемента по ключу.  
            ...
            - Возвращает **итератор** на найденный элемент.
            - Если элемент не найден → возвращает `end()`.
            - В `map` ключи уникальны, поэтому будет найден максимум один элемент.
            - В `multimap` ключи могут повторяться, и `find` возвращает **первый элемент** с данным ключом.

            ---
            */
            std::map<int, std::string> m_find145;
            m_find145[1] = "one";
            m_find145[2] = "two";

            auto it_find145 = m_find145.find(2);
            if (it_find145 != m_find145.end()) {
                std::cout << "Найдено: " << it_find145->first << " -> " << it_find145->second << std::endl;
            }
            else {
            ...
            cout << " ===================================================  Map, Multimap - count(key) ===============================================" << endl << endl;
            /*
            В `std::multimap` метод `count(const Key& key)` возвращает количество элементов с данным ключом.  

            ### Подробности:
            - В отличие от `std::map`, где ключи уникальны, в `multimap` один и тот же ключ может встречаться несколько раз.  
            - Функция `count(key)` перебирает все элементы с этим ключом и возвращает их количество.  
            - Если ключ отсутствует, результат будет `0`.
            */

            std::multimap<int, std::string> mm145count;

            mm145count.insert({ 1, "apple" });
            mm145count.insert({ 2, "banana" });
            mm145count.insert({ 1, "orange" });
            mm145count.insert({ 1, "grape" });

            std::cout << "Количество элементов с ключом 1: "
                << mm145count.count(1) << std::endl;

            std::cout << "Количество элементов с ключом 2: "
                << mm145count.count(2) << std::endl;
            ...
            */
            ...
            cout << " ===================================================  Map, Multimap - upper_bound(key) ===============================================" << endl << endl;
            /*
            В `std::multimap` метод **`upper_bound(const Key& key)`** возвращает итератор на **первый элемент, чей ключ строго больше (`>`) указанного**.  

            ### Отличие от `lower_bound`:
            ...
            */
            ...
            cout << " ===================================================  Map, Multimap - rend() ===============================================" << endl << endl;
            /*
            В `std::multimap` метод **`rend()`** возвращает **обратный итератор**, который указывает на позицию **перед первым элементом контейнера**.  
            ...
            - **Что именно означает**:  
              - `rend()` используется вместе с `rbegin()` для задания диапазона обхода в обратном порядке: `[rbegin(), rend())`.  
              - Сам по себе итератор `rend()` не указывает на реальный элемент, а служит "границей" перед началом контейнера.  
              - Разыменовывать `rend()` нельзя — это приведёт к неопределённому поведению.  
            */

            std::multimap<int, std::string> mm145rend;
            mm145rend.insert({ 2, "two" });
            mm145rend.insert({ 1, "one" });
            mm145rend.insert({ 3, "three" });

            // Обход в обратном порядке
            for (auto it145rend = mm.rbegin(); it145rend != mm.rend(); ++it145rend) {
                std::cout << it145rend->first << " : " << it145rend->second << std::endl;
            }

            cout << endl << endl;

            cout << " ===================================================  Map, Multimap - size() ===============================================" << endl << endl;
            /*
            В `std::multimap` метод **`size()`** возвращает количество элементов, хранящихся в контейнере.  

            ### Основные моменты:
            - **Тип возвращаемого значения**:  
              `size_type` (обычно `std::size_t`).  

            - **Что именно означает**:  
              - Показывает общее количество пар *ключ–значение* в контейнере.  
              - В отличие от `count(key)`, который возвращает количество элементов с конкретным ключом, `size()` даёт общее число всех элементов.  
              - Если контейнер пуст, `size()` возвращает `0`.
            */
            std::multimap<int, std::string> mm145size;
            mm145size.insert({ 1, "one" });
            mm145size.insert({ 2, "two" });
            mm145size.insert({ 2, "another two" }); // одинаковый ключ
            ...
            */
			...
			cout << "The other examples" << endl << endl;
        ...
        case(146):
        {
            ...
            cout << "\tstd::stack называется адаптером контейнера, который может действовать как оболочка стандартных контейнеров," << endl << endl <<
                ...
                "\tили std::deque внизу.std::stack может быть инициализирован другим объектом stack или совместимыми контейнерами" << endl << endl <<
                "\tпоследовательностей, такими как deque, vector и list.Однако обратите внимание, что контейнер по умолчанию," << endl << endl <<
                "\tиспользуемый для хранения элементов stack, -это deque.Кроме того, отсутствует конструктор, поддерживающий" << endl << endl <<
            ...
            /*
            ## 📌 Основные функции `std::stack`

            |           Функция                |                        Назначение                                     |        Пример             |
            |----------------------------------|-----------------------------------------------------------------------|---------------------------|
            | **`empty()`**                    | Проверяет, пуст ли стек. Возвращает `true`/`false`.                   | `if (st.empty()) { ... }` |
            | **`size()`**                     | Возвращает количество элементов в стеке.                              | `std::cout << st.size();` |
            | **`top()`**                      | Доступ к верхнему элементу (без удаления).                            | `int x = st.top();`       |
            | **`push(const T& value)`**       | Добавляет элемент в стек (копированием).                              | `st.push(10);`            |
            | **`push(T&& value)`** *(C++11)*  | Добавляет элемент перемещением.                                       | `st.push(std::move(obj));`|
            | **`emplace(args...)`** *(C++11)* | Конструирует элемент прямо в стеке.                                   | `st.emplace(42);`         |
            | **`pop()`**                      | Удаляет верхний элемент (без возврата значения).                      | `st.pop();`               |
            | **Конструкторы**                 | Создание пустого стека или из контейнера (`deque`, `vector`, `list`). | `std::stack<int> st;`     |
            | **Оператор присваивания**        | Копирование или перемещение стека.                                    | `st2 = st1;`              |
            | **swap()**                       | Обменивает содержимое двух стеков.                                    | `st1.swap(st2);`          |
            */
        ...
        case(147):
        {
            ...
            /*
            В стандартной библиотеке C++ контейнера **`std::priority_queue`** нет "команд" в привычном смысле, но есть набор методов (членов класса), которые позволяют управлять очередью с приоритетами. Вот полный список основных операций:

            ### Основные методы `std::priority_queue`
            - **`empty()`** — проверяет, пуста ли очередь.
            - **`size()`** — возвращает количество элементов.
            - **`top()`** — доступ к элементу с наибольшим приоритетом (верхушка).
            - **`push(const T& value)`** — добавляет элемент.
            - **`emplace(args...)`** — конструирует элемент на месте и добавляет его.
            - **`pop()`** — удаляет элемент с наибольшим приоритетом.
            - **`swap(priority_queue& other)`** — меняет содержимое двух очередей местами.

            ### Конструкторы
            - **`priority_queue()`** — создаёт пустую очередь.
            - **`priority_queue(const Compare& comp)`** — создаёт очередь с заданным компаратором.
            - **`priority_queue(const Compare& comp, const Container& cont)`** — создаёт очередь на основе контейнера.
            - **`priority_queue(InputIterator first, InputIterator last)`** — создаёт очередь из диапазона элементов.

            ### Параметры шаблона
            ```cpp
            template<
                class T,                        // тип элементов
                class Container = std::vector<T>, // контейнер хранения
                class Compare = std::less<typename Container::value_type> // компаратор
        ...
        case(148):
        {
            ...
            cout << "\t================================================================================================================" << endl << endl;
            /*
            **Краткий ответ:** **`std::sort` — это стандартный алгоритм из `<algorithm>`, реализующий быструю сортировку/гибридный 
            алгоритм и принимающий опциональный бинарный предикат (например, лямбда) для сравнения двух элементов; 
            вызов `sort(people_148.begin(), people_148.end(), [](const Person_148 &p1, const Person_148 &p2){ return p1.Score < p2.Score; });
            ` корректно сортирует вектор по полю `Score`.** 

            ### Что делает `std::sort`
            - **`std::sort` — стандартный алгоритм из `<algorithm>`** для сортировки диапазона [first, last). 
            Он использует эффективный гибридный алгоритм (обычно introsort: quicksort + heapsort + insertion sort) и имеет среднюю временную сложность **O(n log n)**. 
            - **По умолчанию** сравнение выполняется через оператор `<`. 
            Если нужен другой порядок, передаёте **бинарный предикат** (функцию, функтор или лямбду), который принимает два элемента и возвращает `true`, 
            если первый должен идти перед вторым. 

            ### Как работает лямбда-компаратор
            - Сигнатура лямбды должна быть эквивалентна `bool cmp(const T& a, const T& b)`.
            - Лямбда возвращает `true`, когда **`a` должно предшествовать `b`** в итоговом порядке.  
            - Пример для сортировки по `Score` по возрастанию:
            ```cpp
            sort(people_148.begin(), people_148.end(),
                 [](const Person_148 &p1, const Person_148 &p2) {
            ...
            ```
            **Важно:** предикат должен задавать строгий слабый порядок (strict weak ordering). 

            ### Отличие от функторов и operator()
            - В вашем коде класс `Person_148` содержит `bool operator()(const Person_148 &p148)` — это **функтор**, который принимает один аргумент и возвращает `bool` 
            (например, для `std::find_if` или `std::count_if`), но **не** подходит как бинарный компаратор для `std::sort`. 
            Для `std::sort` нужен бинарный функтор `bool operator()(const Person_148&, const Person_148&)`. 
            - То есть **лямбда в `sort` — это просто удобный способ передать бинарную функцию сравнения** без объявления отдельного функторного класса. 

            ### Практические советы и подводные камни
            - **Стабильность:** `std::sort` не гарантирует стабильность (равные элементы могут поменять порядок). Если нужна стабильная сортировка, используйте `std::stable_sort`.   
            - **Корректность предиката:** если предикат нарушает транзитивность или симметрию, поведение неопределено — возможны краши или неверный порядок. 
            - **Производительность:** для простых типов встроенный `<` быстрее; для сложных объектов сравнения по ссылке и минимизация копирований важны. 
            */
        ...
        case(149):
        {
            ...
            cout << "\t================================================================================================================" << endl << endl;
            /*
            **Кратко:** **`std::find`, `std::find_if` и `std::find_if_not` — это простые линейные алгоритмы поиска в `<algorithm>`; они возвращают итератор на первый подходящий элемент или `end()` если ничего не найдено.** Примеры в вашем коде корректно показывают проверку `result == v.end()` и использование лямбд/функторов как предикатов; все эти алгоритмы работают с любыми итераторами входного диапазона и требуют C++11+ для лямбд. 

                ---

                ### Основная идея и сигнатуры
                - **`std::find(first, last, value)`** — ищет элемент, равный `value`. Возвращает итератор на найденный элемент или `last`.   
                - **`std::find_if(first, last, pred)`** — возвращает итератор на первый элемент, для которого `pred(element)` истинно.   
                - **`std::find_if_not(first, last, pred)`** — возвращает итератор на первый элемент, для которого `pred(element)` ложно (инвертирует предикат). 

                ---

                ### Сравнительная таблица

                | **Функция** | **Назначение** | **Принимает** | **Возвращает** | **Сложность** |
                |---|---:|---|---|---:|
                | **find** | Поиск по значению | `value` | Итератор на элемент; `last` если не найден | **O(n)**.  |
                | **find_if** | Поиск по условию | унарный предикат | Итератор на первый подходящий; `last` если нет | **O(n)**.  |
                | **find_if_not** | Поиск первого, где условие ложно | унарный предикат | Итератор или `last` | **O(n)**.  |

                ---

                ### Как читать ваш код — ключевые моменты
                - Проверка `if (result == v.end())` — **правильный способ** узнать, найден ли элемент.   
                - В `find_if` вы передаёте лямбду `[](int a){ return a == 7; }` — это **унарный предикат**; `find_if` остановится на первом элементе, где предикат вернёт `true`.   
                - `find_if_not` эквивалентен циклу с `if (!pred(*it)) return it;` — удобно, когда нужно найти первый элемент, **не** соответствующий условию. 

                ---

                ### Поиск в контейнерах пользовательских типов
                - Для `vector<Person149>` вы используете лямбду `[](const Person149 & p){ return p.Score == 51 || p.Name == "Сергей"; }` — это стандартный подход. **Важно**: сравнивайте поля корректных типов (строки/числа).   
                - Функтор `Person149::operator()` в вашем коде делает объект `Person149` вызываемым; такой объект можно использовать как предикат (например, `find_if(people.begin(), people.end(), Person149("x",0))`), но это семантически необычно — обычно создают отдельный предикат или `struct` с нужным условием. 

                ---

                ### Практические советы и подводные камни
                - **Всегда** проверяйте возвращаемый итератор на `end()` перед разыменованием.   
                - Для больших данных или частых поисков рассмотрите **индексы/хеш-таблицы** (`std::unordered_set`/`map`) — они дают амортизированное \(O(1)\) или логарифмическое время вместо линейного.   
                - В C++20 есть диапазоны (`std::ranges::find_if`) с похожей семантикой и удобной интеграцией в пайп-стиль. 

                ---
            */
            vector<int> v = { 8,13,92,6,2,4,10 };
